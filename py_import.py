            ...
            if none_keys:
                ...
                for k, v in row.items():
                    if k and "name" in str(k).lower():
                        ...
                        break
                issues.append(f"Row {i} ({expert_name}): Malformed structure - has {len(none_keys)} None keys")
...
def import_cv_documents(cursor, test_limit=None):
    ...
    if not cv_files:
        print(f"{YELLOW}No CV files found matching pattern ID####.pdf in {cv_dir}{RESET}")
        return 0, 0
    ...
    for expert_id, filename, file_path in cv_files:
        ...
        try:
            ...
            processed_count += 1
            print(f"Imported CV for expert ID {expert_id}: {filename} ({file_size} bytes)")
...
def transform_row(row, expert_areas, cursor):
    ...
    if none_keys:
        ...
        for k, v in row.items():
            if k and "name" in str(k).lower():
                expert_name = v if v else "Unknown"
                break
        print(f"{YELLOW}Warning: Malformed row detected for expert '{expert_name}' - has {len(none_keys)} None keys. Row data: {dict(row)}{RESET}")
    ...
    row_data = {
        ...
        "specialized_area": normalize_specialized_areas_to_ids(get_value("Specialised Area", ""), general_area_text, cursor),
        "is_trained": 1 if get_value("Trained", "No").lower() in ["yes", "y", "true"] else 0,
        "cv_path": None,  # Documents managed via expert_documents table
        "phone": get_value("Phone", "00000000"),
        "email": get_value("Email", "unknown@example.com"),
        "is_published": 1 if get_value("Website", "No").lower() in ["yes", "y", "true"] else 0,
        # Remove the skills field since it doesn't exist in the table
        "approval_document_path": None,  # Documents managed via expert_documents table
...
def normalize_specialized_areas_to_ids(areas_text, general_area_text, cursor):
    ...
    normalization_map = {
        ...
        "Electrical & Electronics Engineering": ["Electrical Engineering", "Electronics Engineering"],
        "Business Management/Business Adminstration": ["Business Management", "Business Administration"],
        ...
        "Interior Architecture / Interior Design": ["Interior Architecture", "Interior Design"],
        "Fashion Technology/ Fashion Design by Computer/ Luxury Fashion Management": ["Fashion Technology", "Fashion Design", "Luxury Management"],
    }
    ...
    for area in unique_areas:
        ...
        if result:
            ...
            # Insert new area
            cursor.execute("INSERT INTO specialized_areas (name, created_at) VALUES (?, CURRENT_TIMESTAMP)", (area,))
...
try:
    ...
    with open(csv_file_path, newline='', encoding='utf-8-sig') as csvfile:
        ...
        for i, row in enumerate(reader, 1):
            ...
            try:
                ...
                if transformed_row:  # Only append if transformation succeeded
                    ...
                    error_log.append(f"Row {i}: Failed to transform row - missing required data")
                    error_count += 1
                    ...
                    try:
                        ...
                        cursor.executemany('''
                            INSERT OR REPLACE INTO experts (
                                ...
                                approval_document_path, original_request_id, updated_at
                            ) VALUES (
                                :id, :name, :designation, :institution, :is_bahraini,
                                :is_available, :rating, :role, :employment_type, :general_area,
                                :specialized_area, :is_trained, :cv_path, :phone, :email, :is_published,
        ...
        cursor.execute(f"UPDATE sqlite_sequence SET seq = {max_id} WHERE name = 'experts'")
        conn.commit()
