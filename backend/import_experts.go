...
func ImportExperts(db *sql.DB, csvFilePath string) error {
	...
	for _, record := range records {
		...
		// Get or generate biography
		biography := getFieldByHeaderName(header, record, "Biography")
		if biography == "" {
			// Auto-generate a simple biography from available fields
			biography = fmt.Sprintf("%s is a %s at %s", name, designation, institution)
			if specializedArea != "" {
				biography += fmt.Sprintf(" specializing in %s", specializedArea)
			}
			biography += "."
		}
		...
		result, err := tx.Exec(
			`INSERT INTO experts (
				...
				is_trained, cv_path, phone, email, is_published, biography, created_at
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
